...
import com.ibdbcompany.ibdb.service.ActionService;
...
/**
 * REST controller for managing {@link com.ibdbcompany.ibdb.domain.Action}.
 */
@RestController
@RequestMapping("/api")
public class ActionResource {
    ...
    /**
     * {@code GET  /actions/search/name} : search all the actions by name.
     *
     * @param name name.
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of actions in body.
     */
    @GetMapping("/actions/search/name")
    public ResponseEntity<List<Action>> getAllActionsByName(@RequestParam String name, Pageable pageable) throws MalformedURLException {
        Page<Action> page = actionService.findAllByName(UrlUtil.decode(name), pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
